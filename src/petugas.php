...
// Query untuk mengambil semua data dari tabel petugas
$query = "SELECT * FROM petugas ORDER BY id_petugas ASC";
$result = mysqli_query($koneksi, $query);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nama_petugas'])) {
  $nama_petugas = mysqli_real_escape_string($koneksi, $_POST['nama_petugas']);
  $query_insert = "INSERT INTO petugas (nama_petugas) VALUES ('$nama_petugas')";
...
<head>
  ...
  <title>Data Petugas | Sistem MR UPT Komputer</title>
...
<body class="bg-gray-50 font-sans min-h-screen flex flex-col">
  ...
  <div class="flex-1 container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
      <h1 class="text-3xl font-bold text-gray-800 mb-1">Daftar Petugas</h1>
      <p class="text-gray-600">Daftar seluruh petugas yang terdaftar dalam sistem.</p>

      <div class="mt-8 flex justify-end">
        <button onclick="openAddModal()"
          class="bg-blue-600 hover:bg-blue-700 text-white py-3 px-3 
               rounded-lg shadow-md inline-flex items-center transition duration-200">
      ...
      <!-- Tabel Petugas -->
      <div class="bg-white rounded-lg shadow-md overflow-hidden mt-2">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <?php if (mysqli_num_rows($result) > 0): ?>
                ...
                while ($row = mysqli_fetch_assoc($result)): ?>
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?= $row['nama_petugas']; ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button onclick="openEditModal('<?= $row['id_petugas']; ?>')" class="text-blue-600 hover:text-blue-900 mr-3">
                        <i class="fas fa-edit mr-1"></i>Edit
                      </button>
                      <button onclick="openDeleteModal('<?= $row['id_petugas']; ?>')" class="text-red-600 hover:text-red-900">
              ...
              <?php else: ?>
                <tr>
                  <td colspan="3" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data petugas.</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
  ...
  <script>
    ...
    function openDeleteModal(id) {
      document.getElementById('deleteLink').href = 'backend/hapus-petugas.php?id=' + id;
  ...
  <div id="addModal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md mx-4 relative">
      <button onclick="closeAddModal()" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
        <i class="fas fa-times text-xl"></i>
      </button>

      <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Tambah Petugas</h2>

      <form method="POST" action="backend/tambah-petugas.php">
        <div class="mb-4">
        ...
        <div class="flex justify-end gap-3 mt-6">
          <button type="button" onclick="closeAddModal()"
            class="px-5 py-2 bg-gray-300 hover:bg-gray-400 text-gray-700 rounded-lg">
            Batal
          </button>
          <button type="submit"
  ...
  <div id="modalDelete" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center hidden">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
      <div class="p-6">
        <div class="flex items-center justify-center w-16 h-16 mx-auto bg-red-100 rounded-full">
          <i class="fas fa-exclamation-triangle text-red-600 text-2xl"></i>
        </div>
        <h3 class="text-lg font-medium text-gray-900 mt-4 text-center">Konfirmasi Hapus</h3>
        <p class="text-sm text-gray-500 mt-2 text-center">Apakah Anda yakin ingin menghapus petugas ini? Tindakan ini tidak dapat dibatalkan.</p>
  ...
  <div id="modalEdit" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-96">
      <h2 class="text-lg font-semibold mb-4">Edit Nama Petugas</h2>
      <form id="editForm" method="POST" action="edit-petugas.php">
        <input type="hidden" name="id_petugas" id="editId">

        <div class="mb-3">
          <label class="block text-sm font-medium">Nama Petugas</label>
          <input type="text" name="nama_petugas" id="editNama" class="w-full border px-3 py-2 rounded" required>
        </div>

        <div class="flex justify-end mt-4">
          <button type="button" onclick="closeEditModal()" class="px-4 py-2 bg-gray-300 rounded mr-2">Batal</button>
